...
def download(start_date,end_date,department=None,category=None):
    filename = 'Monthly Salary Register'
...
def make_xlsx(data,args, sheet_name=None, wb=None, column_widths=None):
    ...
    ws.append(['Sl No.','Employee No','DATE OF JOINING','Employee Name','Department','Designation','Cost Center','Cost Center Name','Basic+DA Salary','DA','HRA','Conveyance Allowance','Medical Allowance','Special  Allowance','Position Bonus','Gross Salary','Paid Days','Over Time (Hrs)','Arrear Days','Arr.Over Time (Hrs)','Salary Deduction','Shift Days','Basic Salary','DA','HRA','Conveyance Allowance','Medical Allowance','Special  Allowance','Position Bonus','Gross Salary','Incentives','Extra Price','Mold Maintenance','Over Time','Arrears Over Time','Arrears Salary','Shift allowance','Attendance Bonus','Non Production Day Allowance','Earned Leave Encashment','Bonus Yearly','Total Gross Salary','PF','PT','ESI','IT Deduction','LWF','VPF','Insurance','Salary Deduction','Total Deduction','Net Salary'])
    emp= get_data(args)
    for e in emp:
        ws.append(e)

    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=52 )
    ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=52 )
    ws.merge_cells(start_row=3, start_column=1, end_row=3, end_column=52 )
    ws.merge_cells(start_row=4, start_column=1, end_row=4, end_column=8 )
    ws.merge_cells(start_row=4, start_column=17, end_row=4, end_column=22 )
    ws.merge_cells(start_row=4, start_column=9, end_row=4, end_column=16 )
    ws.merge_cells(start_row=4, start_column=23, end_row=4, end_column=42 )
    # ws.merge_cells(start_row=4, start_column=43, end_row=4, end_column=44 )
    ws.merge_cells(start_row=4, start_column=43, end_row=4, end_column=52 )
    border_thin = Border(
    ...
    for header in ws.iter_rows(min_row=1, max_row=2, min_col=1, max_col=52):
        for cell in header:
            cell.font = Font(bold=True)
    for header in ws.iter_rows(min_row=4, max_row=5, min_col=1, max_col=52):
        for cell in header:
            cell.font = Font(bold=True)
            cell.alignment = align_center
    for header in ws.iter_rows(min_row=len(get_data(args))+5, max_row=len(get_data(args))+5, min_col=1, max_col=52):
        for cell in header:
            cell.font = Font(bold=True)
            cell.alignment = align_center
    header_range = ws['A1':ws.cell(row=len(get_data(args))+5, column=52).coordinate]
...
def title1(args):
    ...
    if args['category']:
        data = ["SALARY SHEET OF "+ " FOR THE MONTH OF " +date]
    else:
...
def get_data(args):
    ...
    for ss in salary_slip:
        ...
        for e in emp:
            ...
            total_etg =(ss.gross_pay + inc + ot + aot + arr + sha + ss.attendance_bonus)
            if total_etg:
                total_gross_earned+=total_etg
            row.append(total_etg or '-')
            ...
            row.append(esi or '-')
            row.append("")
            lwf = frappe.db.get_value("Salary Detail",{'parent':ss.name,"salary_component":"Labour Welfare Fund"},['amount']) or 0
            if lwf:
                total_lwf+=lwf
            row.append(lwf or '-')
            vpf = frappe.db.get_value("Salary Detail",{'parent':ss.name,"salary_component":"VPF"},['amount']) or 0
            if vpf:
                total_vpf+=vpf
            row.append(vpf or '-')
